...
function DoctorCard({ doctor, onViewProfile }: { doctor: any; onViewProfile: (id: string) => void }) {
  return (
    <div className="bg-white rounded-2xl border border-stone-200 hover:border-saffron-300 hover:shadow-lg transition-all duration-200 overflow-hidden group">
      {/* Top gradient bar */}
      ...
      <div className="p-6">
        ...
        <div className="flex items-start gap-4 mb-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-saffron-100 to-saffron-200 flex items-center justify-center flex-shrink-0 text-2xl font-bold text-saffron-700">
            {doctor.name.charAt(0).toUpperCase()}
          </div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-base font-bold text-stone-800 truncate">Dr. {doctor.name.replace(/^Dr\.?\s*/i, '')}</h3>
              {doctor.verified && (
            ...
            <span className="inline-block mt-1 text-xs font-medium text-saffron-700 bg-saffron-50 rounded-full px-2.5 py-0.5 border border-saffron-200">
              {doctor.specialization}
            </span>
            <span className="inline-block mt-1 ml-1 text-[10px] font-semibold text-stone-600 bg-stone-100 rounded-full px-2 py-0.5 border border-stone-200">
        ...
        {doctor.qualifications && (
          <p className="text-xs text-stone-500 mb-4 bg-stone-50 rounded-lg px-3 py-2 line-clamp-2">
            <span className="font-medium text-stone-600">Qualifications: </span>
            {doctor.qualifications}
          </p>
        )}
        ...
        <div className="flex items-center justify-between pt-4 border-t border-stone-100">
          ...
          <button 
            ...
            className="bg-saffron-500 hover:bg-saffron-600 text-white text-xs font-semibold px-4 py-2 rounded-xl transition-colors group-hover:shadow-md"
          >
            View Profile
          </button>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
        ...
        <div className="flex-1">
          <p className="text-sm font-semibold text-emerald-800">Patient Portal</p>
          <p className="text-xs text-emerald-600">Logged in as {currentUser?.name || 'Patient'} &mdash; find nearby doctors</p>
        </div>
        {userLocation && (
          <div className="flex items-center gap-1.5 text-xs bg-blue-100 text-blue-700 px-3 py-1.5 rounded-lg border border-blue-200">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-stone-800">Find Doctors near you</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full border border-stone-300 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-saffron-400 focus:border-transparent bg-white"
            />
          </div>
          <div className="relative">
            <Filter className="absolute left-3.5 top-3 w-4 h-4 text-stone-400" />
            <select
              value={filterType}
              onChange={e => { setFilterType(e.target.value); setFilterSpec(''); }}
              className="border border-stone-300 rounded-xl pl-10 pr-8 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-saffron-400 focus:border-transparent bg-white appearance-none min-w-[180px]"
            >
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1 min-w-[220px]">
            <select
              ...
              className="border border-stone-300 rounded-xl px-4 pr-8 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-saffron-400 focus:border-transparent bg-white appearance-none w-full"
            >
          ...
          <div className="relative flex-1 min-w-[180px]">
            ...
            <input
              ...
              className="border border-stone-300 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-saffron-400 focus:border-transparent bg-white w-full"
            />
          </div>
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="text-center py-20">
          ...
          {doctors.length === 0 ? (
            <>
              <h3 className="text-stone-600 font-semibold mb-2">No doctors available yet</h3>
              <p className="text-stone-400 text-sm max-w-sm mx-auto">You're all set! Verified practitioners will appear here once they join the platform.</p>
              <p className="text-stone-400 text-xs mt-3">Check back soon — doctors are joining every day.</p>
            </>
          ) : (
            <>
              <h3 className="text-stone-600 font-semibold mb-2">No doctors match your search</h3>
              <p className="text-stone-400 text-sm">Try a different specialization or clear your search.</p>
              <button
                onClick={() => { setSearch(''); setFilterSpec(''); setFilterType(''); }}
                className="mt-3 text-sm text-saffron-600 hover:text-saffron-700 underline"
              >
                Clear filters
              </button>
            </>
